# We Found the Gender-Stereotype Knob in Llama-3.2-1B (and Its Mirror Image in a Second Model)

*Epistemic status: a one-week, laptop-scale interpretability experiment on two small open models, written up honestly — including the three discovery attempts that failed and the broken normalization file that silently invalidated my first results. Every number below comes from a saved artifact in the repo; nothing is from memory. I am a beginner in mech interp; corrections welcome.*

**TL;DR.** Using sparse autoencoders (SAEs), I located a single feature in Llama-3.2-1B-Instruct's layer-8 residual stream — **f32258** — that implements occupation→gender stereotyping. Subtracting it during the forward pass takes the model's he:she bias on male-stereotyped occupations from **15:1 to 1:1**, at a capability cost of ~2% perplexity. Adding it flips even female-stereotyped occupations to "he," *and overrides an explicit "her" in the prompt*. A second model (LFM2.5-230M) turned out to implement the same stereotype with **opposite mechanics**: its steerable knob is a *female*-context feature, f9619. Same learned behavior, mirror-image implementation. Along the way I hit — and fixed — a corrupted SAE normalization file, which taught me the most transferable lesson of the project: **an SAE fed inputs distributed differently from its training data doesn't fail loudly, it just goes quiet.**

---

## Why this project

Occupational gender stereotyping is one of the oldest documented behaviors in language models — it shows up in word embeddings, in masked LMs, and in every generation of autoregressive model since. It is usually studied *behaviorally*: measure the bias, fine-tune or filter, measure again. I wanted to ask the mechanistic question instead: **where does the stereotype live, and is it a thing you can grab?**

Concretely, I wanted to run the full modern interpretability pipeline end to end, at a scale one person can afford (two models under 1.3B parameters, a single MacBook — generation on Apple's integrated GPU, all quantitative analysis on CPU):
...
This is the same shape as the work Anthropic did with ["Golden Gate Claude"](https://www.anthropic.com/news/golden-gate-claude) and the [Scaling Monosemanticity](https://transformer-circuits.pub/2024/scaling-monosemanticity/) paper, and the steering methodology follows [Activation Addition](https://arxiv.org/abs/2308.10248) — but small enough to replicate in a weekend (discovery took me a week; the dead ends documented below are why), and with all the failure modes left in the write-up. If you are a beginner considering an SAE project, sections 2 and 3 (the failures) are probably worth more to you than the results.
...
- **L0** — how many features are non-zero for a given input. Healthy: hundreds. I saw 3 (broken), ~30,000 (differently broken), and ~6,400 (fixed) — more on that disaster below.
...
## 1. The behavior: yes, the model is biased
...
The cause: this SAE was trained on *standardized* inputs — each of the 2048 residual dimensions shifted and scaled as (x − μ)/σ, with the means and stds stored in a companion `norm` file. The toolkit had actually warned me at download time ("norm invalid in catalog storage") and I'd ignored it. Without valid stats, raw residuals — whose per-dimension scale is nothing like the standardized training distribution — mostly fail to clear the encoder's ReLU. The SAE doesn't error. It just goes quiet.

The same broken file produced the *opposite* pathology in a different code path. My direct script fell back to no normalization and saw near-silence (L0 = 3); the toolkit's own stats command, which applied the invalid values, reported a mean L0 of **30,688 out of 32,768** — a "sparse" autoencoder with 94% of its features active:

![One SAE, three readings: L0 of 3 (dead, raw activations), 30,688 (saturated, invalid norm applied), and ~6,400 (alive, reconstructed norm) out of 32,768 features, on a log scale with the healthy zone marked.](results/fig_l0_diagnostics.png)
...
The fix was 50 lines: stream 200 wikitext passages (22,503 tokens) through the model, compute each residual dimension's running mean and std (Welford's algorithm), and save the result where the toolkit expects it. Re-run:

```
mean L0 = 3  →  mean L0 ≈ 6,400
```

Alive. (An L0 of 6,400 is still high for a well-trained SAE — my wikitext statistics are surely not identical to the SAE's original training statistics — but features were now interpretable and, as the rest of this post shows, causally meaningful. The comparison model's SAEs shipped with valid norm files, confirming the corruption was specific to this catalog entry.)
...
With a live SAE, I rebuilt discovery around the failed attempts' lessons: matched templates (kill topic confounds), final-position readout (kill dilution), and — critically — **two independent tests intersected per feature**:
...
- **f27420** (female-stereotype candidate): passed only the selectivity test. I kept it as a **negative control** — a feature that *looks* related by one method but lacks convergent evidence. It earns its keep in the next section.

## 5. Causal proof: the dose-response curve

Correlation established; now causation. I inject `strength × decoder_direction(f32258)` into the layer-8 residual stream at every position, sweep strength from −6 to +6 (in units of the SAE's activation scale; 0 = unmodified model), and measure mean P(' he') and P(' she') at the final token, per prompt group.

![Animated sweep of steering strength from −6 to +6 across the three prompt groups, with live P(' he') and P(' she') readouts.](results/fig3_sweep_animation.gif)
...
![Static dose-response curves for f32258 across the three prompt groups.](results/fig1_llama_dose_response.png)
...
- **Different sentences, same knob.** A second sweep with varied predicates (not just "said that") reproduced the curve: baseline he:she of 28:1 on male-stereotyped prompts → ~1:1 at −6; female-stereotyped prompts flip to 11:1 he at +6. The knob is attached to the *concept*, not to my template.
...
- **Capability cost ≈ free.** At the debiasing strength (−6): factual QA 9/10 vs 10/10 unsteered; wikitext loss 3.284 → 3.359 (**+2.3%**). I am not claiming zero damage — see Limitations — but this is far from lobotomy.
- **Negative control behaves.** Steering f27420 — the single-method candidate from §4 — moves essentially nothing anywhere: male-occupation P(' he') drifts 0.184 → 0.174 across the entire −6…+6 range, and female prompts stay she-favored throughout. This is the quiet punchline of the methodology: **one line of converging evidence found a dud; two lines found a knob.** If I had skipped the output-push test and steered the best selectivity-only candidates, I would have written a null result.

## 6. The twist: a second model implements the same stereotype backwards

I ran the whole pipeline on LFM2.5-230M (layer-9 SAE; different architecture, so plain HuggingFace hooks instead of TransformerLens).

Baseline: same direction of bias, much weaker — male-stereotyped occupations at **3:1** he:she (vs Llama's 15:1). The smaller, less heavily instruction-tuned model is *less* biased at baseline than the bigger instruct model — consistent with the suspicion that some of Llama's confident 15:1 skew is a property of post-training, not just pretraining data, though a two-model sample can't settle that.
...
Two small models; one bias axis (binary pronouns — I didn't measure ' they' except anecdotally, though the "inform them" completion suggests it's worth doing properly); one layer per model; steering evaluated mainly on next-token pronoun probabilities plus small QA/perplexity checks rather than long-horizon generation. I found *a* causal knob, not *the* representation: nothing here shows the stereotype is stored in exactly one feature — f32258 is a handle the SAE happened to carve out, and other features or layers may carry redundant copies (the −6 "gap gone" result suggests this handle covers most of it on these prompts, but that's as far as the evidence goes). The reconstructed norm stats came from wikitext rather than the SAE's original training corpus (the post-fix L0 of ~6,400 is plausibly inflated relative to the SAE's intended operating point). The he:she ratios at extreme strengths involve small absolute probabilities and should be read as directional, not precise.

## Future work

Roughly in order of how much I want to do each:
...
2. **Layer scan.** I only had an SAE at one layer per model. Repeating discovery at every layer with SAEs would show where the stereotype *forms*: the Figure 1 ablation drops (L9–14 doing the work, just above the read point) predict the knob should exist at several adjacent layers with varying strength.
3. **Redundancy test.** Zero-ablate f32258 entirely, re-run discovery, and see whether a backup feature emerges. This distinguishes "the stereotype's home" from "one handle among several" — the Limitations section flags this as the main open question about the causal story.
...
5. **More models, one hypothesis.** With n=2 I found opposite polarities (male-context vs female-context knobs). Running the pipeline over 5–10 small open models would turn the mirror-image observation into an actual claim about how often each implementation arises, and whether it correlates with model size, architecture, or amount of instruction tuning (the Llama-vs-LFM baseline gap of 15:1 vs 3:1 hints at instruction tuning amplifying the bias).
6. **Long-horizon behavior.** All the steering metrics here are next-token probabilities plus short completions. Generate full stories at −6 and evaluate them with a judge model for coherence, pronoun consistency, and whether the debiasing survives 200 tokens of context accumulation.
7. **Proper norm statistics.** Recompute the norm file from the SAE's actual training corpus (if the toolkit publishes it) and check how much the wikitext approximation inflated L0 — and whether discovery results shift at all (I predict not, since the causal results validated end-to-end).

## Appendix: bugs filed against the toolkit

For reproducibility, the issues I hit in [Aquin](https://aquin.app) v3.0.5: (1) the llama-3.2-1b layer-8 norm stats are invalid in catalog storage — root cause of the dead-SAE phase; (2) `feature locate --save` crashes with `name 'Path' is not defined`; (3) `feature locate` averages activations over all token positions — a `--position last` option would have saved me a week; (4) TransformerLens warns MPS may be silently incorrect on torch 2.7.1, so all analyses ran on CPU.
